...
def filter_words(words, skip_words):
    ...
    filtered = words.copy() #copy words list sent into function into new list filtered
    for unfiltered in words: #loops for every unfiltered word in the words list
        if unfiltered in skip_words: #if an unfiltered word is on the filtered list
            filtered.remove(unfiltered) #remove filtered word from the filtered list
    return filtered
...
def normalise_input(user_input):
    ...
    normalised_input = normalised_input.split() #split the string into words
    normalised_input = filter_words(normalised_input, skip_words) #remove useless words
