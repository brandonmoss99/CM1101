...
def remove_punct(text):
    ...
    new_text = ""
    for x in range (0, len(text), 1): #loops through each character in the text passed
        if text[x].isalpha() or text[x].isspace(): #checks if character is letter or a space
            new_text = new_text + text[x] #adds the character to the end of the new text
...
def remove_spaces(text):
    ...
    new_text = text.strip() #strips string of whitespace before and after text
    return new_text
...
def normalise_input(user_input):
    ...
    punct_removed = remove_punct(user_input) #function to remove punctuation
    space_removed = remove_spaces(punct_removed) #function to remove whitespace
    text_lowered = space_removed.lower() #converts text to all lowercase
...
def display_room(room):
    ...
    print() #prints the blankline
    print(room["name"].upper()) #.upper turns room into uppercase
    print()
...
def exit_leads_to(exits, direction):
    ...
    leads_to = exits[direction] #will lookup the next room for a given exit & direction
...
def print_menu(exits):
    ...
    for direction, leads_to in exits.items(): #loop for each direction and leads_to on the exit items
        print_menu_line(direction.upper(), rooms[leads_to]["name"]) #run print line function
...
def is_valid_exit(exits, user_input):
    ...
    True
    """
    try: #will try running the following code
        exits[user_input]
        return True
    except KeyError: #will do this if the error code from trying previous code is this
        return False
